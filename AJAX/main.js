...
function loadData(){
  ...
  // Adresa la care vom trimite request-ul HTTP 
  const requestURL = 'https://mdn.github.io/learning-area/javascript/oojs/json/superheroes.json'; 
  
  // Procedura de trimitere a unui request HTTP prin XMLHttpRequest 
  const request = new XMLHttpRequest(); //instantiaza un obiect pe constructorul API
  ...
  // Caracteristica 3) a API-urilor: folosesc evenimente pentru a gestiona schimbarile
  // de stare
  // Mai jos, se ataseaza un event handler care va fi executat in momentul primirii 
  // unui raspuns
  
  request.onload = function () {
      let superHeroes = request.response; // obiectul JS primit ca raspuns
...
// Functie care scrie in tag-ul <header>
